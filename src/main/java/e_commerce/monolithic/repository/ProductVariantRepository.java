...
public interface ProductVariantRepository extends JpaRepository<ProductVariant, Long> {
    ...
    @Query("SELECT COUNT(pv) > 0 FROM ProductVariant pv WHERE pv.size.id = :sizeId")
    ...
    @Query("SELECT COUNT(DISTINCT pv.product) FROM ProductVariant pv WHERE pv.size.id = :sizeId")
